...
ok(normalize('مَكتَب طرابلُس المركز') === normalize('مكتب طرابلس المركز'), 'مطابقة الأسماء مع التشكيل');
...
const csv = [
  'الاسم الأول;اسم الأب;اللقب;تاريخ الميلاد;الرقم الوطني;الجنس;محل الإقامة;الهاتف;القسم;الحلقة;المركز;المكتب;المستوى;المتون;مقدار الحفظ;ملاحظات',
  'محمد;أحمد;الشريف;12/03/2005;١١٩٨٧٠٠٠١٢٣٤;ذكر;طرابلس;091-234-5678;قسم الرجال;حلقة النور;مركز الفتح;مكتب طرابلس المركز;المستوى الأول;"الأربعون النووية، عمدة الأحكام";المتن كاملاً;"سطر أول',
  'سطر ثانٍ"',
  'فاطمة;علي;المبروك;2008-01-20;120080001111;أنثى;تاجوراء;0923334444;;حلقة الهدى;مركز التوحيد;مكتب تاجوراء;المستوى الثاني;تحفة الأطفال;نصف المتن;',
  'خالد;سالم;القمودي;01/01/2004;120040002222;ذكر;جنزور;0945556666;قسم الرجال;حلقة البيان;مركز الرسالة;مكتب غير موجود;المستوى الأول;متن غير موجود;الجزء الأول;',
  'مكرر;في;الملف;01/01/2004;119870001234;ذكر;طرابلس;0912345678;قسم الرجال;حلقة;مركز;مكتب طرابلس المركز;المستوى الأول;تحفة الأطفال;كامل;',
  '',
].join('\r\n');

const { header, rows } = parseCsv('﻿' + csv);
ok(rows.length === 4, `قراءة 4 صفوف (الفاصلة المنقوطة، الاقتباس، سطر داخل خلية) → ${rows.length}`);
ok(rows[0][15] === 'سطر أول\r\nسطر ثانٍ', 'سطر جديد داخل خلية مقتبسة');
...
const lookups = { offices: await lookup('offices'), levels: await lookup('levels'), matns: await lookup('matns') };
const built = rows.map((cells) => buildPayload(cells, index, lookups));
ok(built[0].errors.length === 0 && built[0].payload.national_id === '119870001234' && built[0].payload.matn_ids.length === 2,
  'الصف 1: أرقام عربية ومتنان مفصولان بفاصلة عربية');
...
ok(built[2].errors.some((e) => e.includes('المكتب غير معروف')) && built[2].errors.some((e) => e.includes('المتن غير معروف')),
...
const saved = (await db.query(`select s.full_name, a.section, a.student_notes,
  (select count(*) from application_matns m where m.application_id = a.id)::int matns
  from applications a join students s on s.id = a.student_id order by a.created_at`)).rows;
ok(saved.length === 2 && saved[0].matns === 2 && saved[1].section === 'women', 'البيانات المحفوظة صحيحة (المتون والقسم)');
